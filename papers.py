...
2. Read through all the docstrings in this file once. There is a lot to take in,
   ...
   not that you are dateng it in a particular way. We will access your class
...
class PaperTree(TMTree):
    ...
    === Private Attributes ===
    _auths:
        The auths of this paper in list
    _date:
        the date of this paper
    These should store information about this paper's <auths> and <date>.
    ...
    def __init__(self, name: str, subtrees: List[TMTree], auths: str = '',
                 date: str = '', citations: int = 0, by_year: bool = True,
        ...
        <auths> and <date>, and with <citations> as the size of the data.
...
def _get_data(num: int) -> Union[int, str]:
    """
...
def _lptd_helper(d: Dict, category: List) -> None:
    if category == []:
        pass
    top = category[0]
    if top not in d:
    ...
    if not by_year:
        ...
        with open(DATA_FILE, newline='') as csvfile:
            ...
            for row in load_file:
                categories.append(row[3].split(': ') + [load_file.line_num])
        for category in categories:
            _lptd_helper(d, category)
        return d
    else:
        d = {}
        categories = []
        with open(DATA_FILE, newline='') as csvfile:
            ...
            next(load_file)
            for row in load_file:
                categories.append([row[2]] + row[3].split(': ') + [
                    load_file.line_num])
        for category in categories:
            _lptd_helper(d, category)
        return d
